Feature: Maintenance Order Cancellation
  ...
  Scenario: Cancellation of a completed maintenance order with error
    When the clerk approves and completes the maintenance order with id "3"
    Then the status of the maintenance order is "COMPLETED"
    When the site engineer "Jacob" cancels the completed maintenance order with id "3" from RentIT
